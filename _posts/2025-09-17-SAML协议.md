...
SAML、OAuth 2.0、OIDC 三者经常一起被提到，但各自解决的问题不同。SAML 解决跨域身份认证，回答"这个用户是谁"；OAuth 2.0 解决授权委托，回答"这个应用能代表用户访问哪些资源"；OIDC 构建在 OAuth 2.0 之上，既继承了 OAuth 2.0 的授权能力，又补充了标准化的身份认证层，因此同时覆盖认证和授权。三者的关系可以概括为：SAML 只做身份认证，OAuth 2.0 只做授权，OIDC 两者都做。
...
sequenceDiagram
    ...
    User->>SP: 访问受保护资源
    SP-->>User: 重定向到 IdP，并携带 AuthnRequest
    User->>IdP: 发起认证请求
    IdP->>IdP: 校验用户身份
    IdP-->>User: 返回带有 SAML Response 的表单
    User->>SP: POST 到 ACS URL
    SP->>SP: 验签、校验断言、建立本地会话
    SP-->>User: 登录成功并返回目标页面
```
...
1. 用户访问 SP 的受保护资源。
2. SP 发现没有本地会话，生成 `AuthnRequest`。
3. SP 通过浏览器把用户重定向到 IdP 的 SSO 端点。
4. IdP 完成用户认证。
5. IdP 生成 `SAML Response`，其中包含已签名的断言。
...
sequenceDiagram
    ...
    User->>IdP: 已登录门户，点击"进入业务系统"
    IdP->>IdP: 生成 SAML Response（含已签名断言）
    IdP-->>User: 返回带有 SAML Response 的自动提交表单
    User->>SP: POST 到 SP 的 ACS URL（含 Response 和 RelayState）
    SP->>SP: 验签、校验断言、建立本地会话
    SP-->>User: 跳转到目标页面
```
...
2. IdP 直接生成包含已签名断言的 `SAML Response`，无需等待 SP 的请求。
3. IdP 通过浏览器将该响应以 HTTP POST 的方式提交到 SP 的 ACS URL，通常还会附带 `RelayState` 指示目标页面。
4. SP 收到响应后，执行与 SP 发起流程相同的校验：验签、校验 `Issuer`/`Audience`/时间窗口等。
5. 校验通过后建立本地会话，将用户跳转到 `RelayState` 指定的目标页面。

与 SP 发起方式相比，IdP 发起流程省去了 `AuthnRequest` 的生成和重定向环节，SP 的处理逻辑更简单，但工程上有几点需要额外关注：

- **没有 `InResponseTo`**：SP 发起方式中，`Response` 会携带对应的 `AuthnRequest` ID（即 `InResponseTo`），SP 可以验证响应是否来自本次请求。IdP 发起时这个字段为空，SP 无法做请求绑定校验，防重放依赖其他手段（如断言 ID 去重）。
...
<EntityDescriptor entityID="https://idp.example.com/SAML2">
    <IDPSSODescriptor>
        <KeyDescriptor use="signing">
            <X509Certificate>MIICxDCCA...</X509Certificate>
        </KeyDescriptor>
        <SingleSignOnService
...
手工填写参数也能完成对接，但生产环境更适合让端点、证书、实体标识、绑定方式保持成组一致。元数据的价值在于减少人工拼装配置带来的错配风险。不同阶段的系统适合不同的交换策略，选择标准是风险控制和运维能力。

| 阶段 | 交换方式 | 核心特点 | 适用判断 |
|------|----------|----------|----------|
| **POC / 测试** | 手动配置参数 | 快速验证，改动直接 | 适合短周期验证，不适合长期维护 |
| **生产首次对接** | 线下交换元数据文件 | 人工审核、控制面清晰 | 适合首次建立信任或高安全要求场景 |
| **生产稳定运行** | 元数据 URL 自动获取 | 自动同步、利于证书轮换 | 适合运维成熟、对自动化有要求的环境 |

### 4.1 手动配置参数

这种方式本质上是把元数据中的关键项拆开后逐项填写。优点是快，缺点是容易漏项和错配。
...
这是生产首次对接中最常见、也最稳妥的方式。双方导出元数据，通过可信渠道交换并人工审核。
...
系统进入稳定运行阶段后，元数据 URL 能显著降低证书轮换和配置变更的维护成本。
...
元数据交换和 SAML 消息传递都依赖同一套底层安全机制：数字签名提供来源可信和防篡改保证，证书承载公钥并建立信任锚点，校验逻辑确保接收方只接受合法的消息和断言。
...
这一机制同时适用于 SAML 消息签名（如 IdP 对 Response 和 Assertion 签名）和元数据签名（如 IdP/SP 对自身元数据文档签名）。
