...
SAML（Security Assertion Markup Language）是基于 XML 的身份联合协议，用于在身份提供方（IdP）和服务提供方（SP）之间交换身份信息。

它最常见的用途是跨系统单点登录（SSO）。用户在 IdP 完成认证后，可以进入多个 SP，无需每个系统都重新登录。企业内部统一登录、跨组织身份联合和传统 Web 应用接入都能看到它。

SAML、OAuth 2.0 和 OIDC 经常出现在同一套身份架构中，分工并不一样。SAML 处理身份联合，OAuth 2.0 处理授权委托，OIDC 在 OAuth 2.0 之上提供标准化的身份认证和用户声明。

---

## 2. SAML 认证流程

SAML 认证涉及用户浏览器、IdP 和 SP。流程中会传递 `AuthnRequest`（认证请求）、`Response`（协议响应）、`Assertion`（身份声明）和 `RelayState`（登录前上下文）。SP 最终消费的是 `Assertion`，只有完整校验通过后才会接受登录结果。
...
浏览器负责传递 SAML 消息，IdP 与 SP 之间的证书和配置负责建立信任。
...
- `Issuer` 是否与受信任的 IdP 一致。
- `NameID` 或属性映射能否识别本地用户。
- `NotBefore`、`NotOnOrAfter` 是否通过时间校验。
- `Audience` 是否匹配当前 SP 的 `Entity ID`。
...
IdP 发起流程省去了 `AuthnRequest` 和首次重定向，SP 还要处理几个额外问题：
...
链路跑通后，SP 仍要决定是否信任这次登录。判断依据来自双方交换的元数据和预先配置的证书。

### 3.1 元数据是什么

元数据是 SAML 双方的机器可读配置，记录实体标识、端点、证书和支持的绑定方式。
...
`Entity ID`、`SSO URL`、`ACS URL`、签名证书和绑定方式必须在 IdP 与 SP 之间保持一致。联调时可以手工填写，生产环境更适合成组维护元数据，减少漏项和错配。交换方式要根据系统阶段和运维能力选择。
...
联调初期可以把元数据中的关键项拆开，逐项手工填写。这种方式配置快，也容易漏项和错配。
...
生产首次对接需要建立可审核的信任关系，常见做法是双方导出元数据，通过可信渠道交换，再由人工审核。
...
系统稳定运行后，运维重点会转到证书轮换和端点变更。使用元数据 URL 可以减少手工同步。
...
SAML 的信任依赖数字签名、证书和完整的校验逻辑。签名证明来源并防止篡改，证书承载公钥，接收方再按协议约束检查消息和断言。
...
签名验证确认来源与完整性，断言条件校验确认有效期和适用范围，两项都要通过。

---

## 6. 安全最佳实践

登录成功只说明主链路已经跑通。证书轮换、时间偏差、重放防护和异常配置决定它能否长期稳定运行。
