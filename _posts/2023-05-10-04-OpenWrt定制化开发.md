...
categories: [SD-WAN]
---
本文分析 OpenWrt 首次启动时的板级识别、网络配置生成、MAC 地址读取和默认配置写入流程。

## 一、为什么需要定制化

SD-WAN 终端固件需要预置以下设备初始化参数：
...
这些默认项应写进固件构建和首次启动流程，避免出厂后逐台手工配置。

## 二、网络配置的自动生成流程

系统启动时，`S10boot` 会调用 `/bin/config_generate` 生成初始网络配置。下面展开这条调用链。
...
设备树（Device Tree Source）描述 GPIO 引脚映射、Flash 和内存大小、LED 与按键位置等硬件参数。
