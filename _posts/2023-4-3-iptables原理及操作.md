...
iptables 规则是否生效，取决于报文路径、挂载链、表的处理阶段和规则顺序。排查时需要先确定：

- 报文从哪里来，要到哪里去。
- 它会在内核里的哪些位置被检查。
- 各链上的规则以什么顺序完成过滤、地址转换或报文标记。

本文先说明报文如何经过表和链，再整理常用操作，最后解释 iptables、nftables、ufw 和 firewalld 的关系。

主机防火墙与网关防火墙处理的流量路径不同。
...
`Netfilter` 和 `iptables` 分属内核态与用户态。

`Netfilter` 是 Linux 内核的报文处理框架，在网络栈的入站、路由、转发和出站路径上提供 Hook。规则可以在这些 Hook 上检查、修改、放行或丢弃报文。

`iptables` 是在命令行中配置规则的工具，`Netfilter` 才是内核中实际检查和处理报文的框架。报文进入、转发或离开 Linux 主机时，会经过若干 Netfilter 检查点，iptables 负责告诉内核各检查点应如何处理流量。

下图给出五条内置链在报文路径中的位置。链决定处理位置，表决定规则用途。
...
连接跟踪模块维护报文与连接状态的映射，规则可以直接匹配连接阶段，无需分别枚举请求和响应方向。常见状态包括：
