...
categories: [SD-WAN]
---
设备 Agent 负责控制器通信、配置下发、隧道管理和分流规则执行。本文记录该程序在 32 MB Flash、128 MB RAM 的 MIPS 路由器上的交叉编译、依赖裁剪和进程守护。
...
目标设备使用 MIPS 处理器，开发机通常是 x86_64。开发机直接编译的二进制无法运行在 MIPS 上，需要通过交叉工具链生成目标平台程序。
...
Agent 可以通过 UBUS 实时获取接口的 UP/DOWN 状态和流量统计，用于管理隧道与监控链路。
