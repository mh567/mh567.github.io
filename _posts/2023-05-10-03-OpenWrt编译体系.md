---
layout: post
title: SD-WAN终端研发系列03 OpenWrt编译体系：从源码到固件的全链路
categories: [SD-WAN]
---
本文梳理 OpenWrt 从源码、Feeds 和配置到固件镜像的构建链路，并说明内核、软件包和设备配置的定制入口。
...
`build_dir` 是实际编译工作区。源码从 `dl` 解压到这里，`configure`、`make` 和 `make install` 都在此目录执行。`make clean` 会清空该目录，直接修改其中的文件无法长期保留，应通过补丁维护变更。
...
Toolchain 是交叉编译工具集，包含在 x86 主机上编译 MIPS 或 ARM 程序所需的编译器（gcc）、链接器（ld）和汇编器（as）等工具。
...
*下一篇介绍 OpenWrt 默认配置、网络初始化和 MAC 地址的定制方法。*
