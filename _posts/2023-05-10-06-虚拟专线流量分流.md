...
但域名的IP地址可能变化（CDN场景下尤其常见），所以不能用一次性DNS查询解决——需要**持续监听DNS响应，将解析出的IP动态加入ipset**。
...
L2TP隧道本质上是虚拟网络接口。如果隧道断线，对应的网络接口（如`tunnel0`）会变为DOWN状态，与之关联的路由规则会自动消失。隧道重新连接后，接口恢复UP状态，但路由规则不会自动恢复。
